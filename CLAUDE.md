...
`bootstrap.sh` handles the full setup: Homebrew, brew packages (neovim, tmux, fzf, lazygit, go, etc.), Oh-My-Zsh + plugins, TPM, NVM + Node.js, Python tools (black, debugpy), Rust + stylua, Go toolchain, symlinks, default shell, and headless plugin installs. Mason + mason-tool-installer auto-install LSP servers, formatters, and debuggers on first launch. The `linux` branch has a WSL/Ubuntu variant using apt.
