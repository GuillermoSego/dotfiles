...
`bootstrap.sh` handles the full setup: Homebrew, brew packages (neovim, tmux, fzf, lazygit, etc.), Oh-My-Zsh + plugins, TPM, NVM + Node.js, Python tools (black, debugpy), Rust + stylua, symlinks, default shell, and headless plugin installs. The `linux` branch has a WSL/Ubuntu variant using apt.
